...
async function doSearch() {
  ...
  if (currentTable === 'events') {
    let query = sb.from('events').select('*, profiles(display_name, university, ambassador_id, phone)');
...
function renderProfileRows(rows) {
  ...
  c.innerHTML = `<div class="exec-table-wrap"><table class="exec-table">
    ...
    <tbody>${rows.map(r=>`<tr data-id="${esc(r.id)}"><td>${esc(r.display_name||'—')}</td><td>${esc(r.ambassador_id||'—')}</td><td>${esc(r.university||'—')}</td><td>${r.status==='inactive' ? `<span class="flag-badge" style="background:#fee2e2;color:#b91c1c;">Inactive${r.inactive_until?' → '+esc(r.inactive_until):''}</span>` : '<span class="flag-badge" style="background:#dcfce7;color:#15803d;">Active</span>'}</td></tr>`).join('')}</tbody>
  </table></div>`;
  c.querySelectorAll('tr[data-id]').forEach(tr => tr.addEventListener('click', () => openEditor(rows.find(r=>r.id===tr.dataset.id))));
}
...
async function confirmSave() {
  ...
  // Refresh the record.
  const sel = currentTable === 'events' ? '*, profiles(display_name, university, ambassador_id, phone)' : '*';
