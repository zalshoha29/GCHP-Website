...
function renderProfileRows(rows) {
  ...
  c.innerHTML = `<div class="exec-table-wrap"><table class="exec-table">
    <thead><tr><th>Name</th><th>Ambassador ID</th><th>University</th><th>Phone</th></tr></thead>
    <tbody>${rows.map(r=>`<tr data-id="${esc(r.id)}"><td>${esc(r.display_name||'—')}</td><td>${esc(r.ambassador_id||'—')}</td><td>${esc(r.university||'—')}</td><td>${esc(r.phone||'—')}</td></tr>`).join('')}</tbody>
  </table></div>`;
  c.querySelectorAll('tr[data-id]').forEach(tr => tr.addEventListener('click', () => openEditor(rows.find(r=>r.id===tr.dataset.id))));
}
